...
To load a OpenTiny editor similar to the xref:basic-example.adoc[Basic example], add the following code to an empty HTML file.
ifeval::["{productSource}" == "package-manager"]
[source,html,subs="attributes+"]
----
<!DOCTYPE html>
<html lang="en">
  ...
  <body>
    <div>
      <textarea id="tiny">&lt;p&gt;Welcome to the OpenTiny jQuery example!&lt;/p&gt;</textarea>
    </div>
    <script>
...
== OpenTiny in a jQuery UI Dialog
