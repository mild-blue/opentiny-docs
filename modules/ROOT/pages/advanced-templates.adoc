...
. Select the template to add to the OpenTiny document from the _Templates_ dialog that presents.
. Click *Insert* or press *Return*.

=== Adding a new template
To add a new template

. Select the material in the OpenTiny document that is going to be the new template and
