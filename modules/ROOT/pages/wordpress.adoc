= WordPress integration
:navtitle: WordPress
:description: Add OpenTiny to WordPress
:keywords: integration, integrate, wordpress, advanced

include::partial$misc/admon-third-party-integration.adoc[]

To use {productname} in WordPress, Advanced Editor Tools is a great option.

== Advanced Editor Tools

https://wordpress.org/plugins/tinymce-advanced/[Advanced Editor Tools] is a WordPress plugin built by https://profiles.wordpress.org/azaozz/[Andrew Ozz] that will let you add, remove, and arrange the buttons that are shown on the Visual Editor toolbar.

*Some of the features added by this plugin*

* Support for creating and editing tables
* More options when inserting lists
* Search and Replace in the editor
* Ability to set Font Family and Font Size

For details, visit:

* https://wordpress.org/plugins/tinymce-advanced/[WordPress.org Plugins - Advanced Editor Tools].
* link:{blogurl}/how-to-use-tinymce-premium-plugins-in-wordpress-5[{companyname} Blog - How to use OpenTiny premium plugins in WordPress 5].
