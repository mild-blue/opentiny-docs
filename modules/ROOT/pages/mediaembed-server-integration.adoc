...
See the docs on xref:mediaembed-server-config.adoc#configure-a-custom-endpoint[configuring a custom endpoint] for details on getting the Enhanced Media Embed server to utilize your custom API.

=== OEmbed endpoint

This is a popular choice and many CMSs have existing plugins that support oEmbed. While you can create custom HTML embeds this way, they cannot contain scripts. If the HTML contains a script, then the Enhanced Media Embed server will embed a xref:mediaembed-server-config.adoc#summary-cards[summary card].
...
=== OpenTiny Enhanced Media Embed endpoint

The other option for developing a custom API endpoint is to return JSON in the xref:tinymce-enhanced-media-embed-format[{productname} Enhanced Media Embed format].
...
=== OpenTiny Enhanced Media Embed format

==== HTTP response status codes

* HTTP 200 (OK): xref:ephoxembedobj[`+EphoxEmbedObj+`]
* HTTP 400 (User Error): xref:errorobj[`+ErrorObj+`]
* HTTP 503 (Upstream Error): xref:errorobj[`+ErrorObj+`]
* HTTP 500 (Unexpected Error): xref:errorobj[`+ErrorObj+`]
...
* `+links+` (required)
** xref:linksobj[LinksObj]
* `+rel+` (optional)
** xref:relobj[RelObj]
* `+media+` (optional)
** xref:mediaobj[MediaObj]
* `+html+` (optional)
...
* `+primary+` (required)
** Array of xref:primaryrel[PrimaryRel]s
* `+technical+` (required)
** Array of xref:technicalrel[TechnicalRel]s
* `+source+` (required)
** Array of xref:sourcerel[SourceRel]s
...
* `+players+` (required)
** Array of xref:linkobj[LinkObj]s
* `+thumbnails+` (required)
** Array of xref:linkobj[LinkObj]s
* `+apps+` (required)
** Array of xref:linkobj[LinkObj]s
...
** Integer
* `+width+` (required)
** xref:dimensionboundobj[DimensionBoundObj]
* `+height+` (required)
** xref:dimensionboundobj[DimensionBoundObj]
